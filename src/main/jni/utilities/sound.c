...
#ifdef _WINDOWS
static void CALLBACK FillSoundBuffer(HWAVEOUT, UINT uMsg, DWORD_PTR dwInstance, DWORD_PTR dwParam1, DWORD_PTR) {
...
int soundinit(AUDIO_t *audio) {
	...
	for (int i = 0; i < BUFFER_SMAPLES; i++) {
...
void KillSound(AUDIO_t* audio) {
	...
	audio->init = FALSE;
#ifdef _WINDOWS
	for (int i = 0; audio->endsnd < BUFFER_BANKS && i < 200; i++) {
		Sleep(5);
	}

	waveOutClose(audio->hWaveOut);
	for (int i = 0; audio->endsnd < 100 && i < 200; i++) {
		Sleep(5);
	}
#endif
}
...
int nextsample(CPU_t *cpu) {
	...
	AUDIO_t* audio = &link->audio;
	double tmp;
	const double max = 255.0f * audio->volume;
	const double lower = (255.0f - max) / 2.0f;
	...
	if (cpu->timer_c->elapsed < (audio->LastSample + SAMPLE_LENGTH)) {
		return 0;
	}
